        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('ruangan_store') }}">
                        @csrf

                        <div>
                            <x-input-label for="kode_ruangan" :value="__('Kode Ruangan')" />
                            <x-text-input id="kode_ruangan" class="block mt-1 w-full" type="text" name="kode_ruangan" :value="old('kode_ruangan')" required autofocus/>
                            <x-input-error :messages="$errors->get('kode_ruangan')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="nama_ruangan" :value="__('Nama Ruangan')" />
                            <x-text-input id="nama_ruangan" class="block mt-1 w-full" type="text" name="nama_ruangan" :value="old('nama_ruangan')" required/>
